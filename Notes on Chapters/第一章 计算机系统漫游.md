...
- 预处理器cpp先根据以字符#开头的命令修改原始C程序。如在hello.c中，cpp根据`#include<studio.h>`命令读取系统头文件`studio.h`中的内容，并将它直接插入程序文本中。预处理的结果是得到一个另一个以.i作为扩展名的C程序。
- 编译器ccl将文本文件hello.i翻译成文本文件hello.s，它包含一个汇编语言程序。该程序包含`main`函数的定义。(注：书上与Ubuntu17.04使用`gcc -S hello.c`命令运行出来的结果不同，见本文尾部附录）
